...
# --- [SECTION 2: FANCY NEON UI & CHAT STYLING] ---
st.markdown("""
    <style>
    ...
    /* TELEMETRY BAR */
    .telemetry { font-family: 'Orbitron', sans-serif; font-size: 0.65rem; color: #00E5FF; text-align: center; letter-spacing: 2px; 
                 background: rgba(0, 229, 255, 0.05); padding: 8px; border: 1px solid rgba(0, 229, 255, 0.1); margin-bottom: 20px; }
    .status-live { color: #00FF41; text-shadow: 0 0 8px #00FF41; font-weight: bold; }
    ...
    .chat-label { font-family: 'Orbitron', sans-serif; color: #00E5FF; font-size: 0.7rem; letter-spacing: 2px; margin-bottom: 10px; border-bottom: 1px solid rgba(0,229,255,0.1); padding-bottom: 5px; }
    
    /* INPUT BAR STYLING */
    .stChatInputContainer { border-radius: 20px !important; border: 1px solid #00E5FF !important; background: rgba(0,0,0,0.8) !important; }
    header, footer { visibility: hidden; }
    </style>
    
    <div class="jarvis-hud"><div class="ring ring-1"></div><div class="ring ring-2"></div></div>
    <div class="telemetry">SYSTEM ENCRYPTION: <span class="status-live">SECURE</span> &nbsp; | &nbsp; STATUS: <span class="status-live">LIVE_OS</span> &nbsp; | &nbsp; UPLINK: <span class="status-live">OPTIMAL</span></div>
    """, unsafe_allow_html=True)

# --- [SECTION 3: MULTILINGUAL CORE & AUDIO] ---
def speak(text):
    try:
...
# --- [SECTION 4: DUAL-INPUT UPLINK (TEXT & VOICE)] ---
if "messages" not in st.session_state: st.session_state.messages = []

# Top Control Row
col1, col2 = st.columns([2,1])
with col1:
    voice_msg = st.audio_input("🎙️ HOLD TO SPEAK (Punjabi/Urdu/Eng/Finnish)")
with col2:
    scan = st.file_uploader("📸 SCAN", type=['png', 'jpg', 'jpeg'])

st.divider()

# Rendering Fancy History
...
# Handle Voice Messaging (Whisper Integration)
if voice_msg:
    try:
        client = OpenAI(api_key=st.secrets["OPENAI_API_KEY"].strip())
        audio_file = io.BytesIO(voice_msg.read()); audio_file.name = "v.wav"
        # Whisper automatically detects English, Urdu, Punjabi, Hindi, and Finnish
...
if prompt:
    ...
    with st.chat_message("assistant"):
        ...
        IDENTITY: Tell users you are Obitwicex, created to show AI's POWER. AI performs, not just thinks.
        MULTILINGUAL: You understand English, Roman Urdu, Punjabi, Hindi, and Finnish. Respond in the user's language.
        PERSONALITY: Elite Lahori Yaar. Witty, sharp, no corporate filters. 
        VOICE: Enabled. Speak out loud."""
