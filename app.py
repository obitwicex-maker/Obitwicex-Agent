...
def transcribe_voice(audio_bytes):
    try:
        ...
        # CLEAN KEY HANDSHAKE (Removes invisible characters/spaces)
        raw_key = st.secrets["OPENAI_API_KEY"].strip().strip('"').strip("'")
        ...
        if "last_audio_id" not in st.session_state or st.session_state.last_audio_id != id(voice_data):
            with st.spinner("🔍 DECODING..."):
                ...
                if transcript != "VOICE_STREAMS_OFFLINE":
                    ...
                    st.error("Authentication Failure. Ensure your $5 credit is active.")
...
if prompt:
    ...
    with st.chat_message("assistant"):
        sys_msg = "You are Obitwicex AI Agent. Elite partner. Use Roman Urdu for chat, English for data. Current time " + datetime.now().strftime('%H:%M')
