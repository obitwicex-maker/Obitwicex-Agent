...
st.markdown("""
    ...
    /* MEDIA DOCK & INPUT FIX (KILLS RED BORDER) */
...
# --- [SECTION 4: UNIFIED MEDIA DOCK] ---
if "messages" not in st.session_state: st.session_state.messages = []

# CONSOLIDATED INPUT DOCK
...
for m in st.session_state.messages:
    with st.chat_message(m["role"]):
        ...
        text_content = m['content'][0]['text'] if isinstance(m['content'], list) else m['content']
...
if prompt:
    final_query = prompt
    if location_uplink: 
        final_query += f" [LOC_DATA: Lahore, Pakistan. Use this context for: {prompt}]"
    ...
    with st.chat_message("assistant"):
        full_reply = ""
        sys_msg = "ROLE: OBITWICEX. Witty Lahori. Multi-lingual. Only reveal creator (Obi) if asked. Use SEARCH:[q] for location results."
        ...
        if "SEARCH:" in full_reply:
            ...
            follow_up = client.chat.completions.create(model="anthropic/claude-3.5-sonnet", messages=[{"role":"system","content":sys_msg}, {"role":"user","content":f"Results: {web_data}. Answer user: {prompt}"}])
